...
int cargar_fecha(const datos_csv_t* datos_entrada, struct tm* fecha, const int columna_csv_anyo,
	...
	const int columna_csv_minuto, const int columna_csv_segundo,const int numero_fila,
  const int incluir_minuto, const int incluir_segundo) {
	/*
	Carga una fecha desde un archivo CSV.
	*/
  ...
  if (incluir_segundo) {
    fecha->tm_sec = strtol(datos_entrada->datos[numero_fila][columna_csv_segundo], NULL, 10);
  }
