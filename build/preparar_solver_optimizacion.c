...
int preparar_solver_optimizacion(problema_optimizacion_t* problema_optimizacion, informacion_procesada_t* informacion_sistema) {
  ...
  printf("El numero de restricciones es %lld \n", problema_optimizacion->numero_restricciones);
  printf("El numero de variables es %lld \n", problema_optimizacion->numero_variables);
  ...
  */
  ...
  OSQPFloat P_x[1] = { 0 };
  OSQPInt P_p[145] = {0 };  // Dummy array (unused for zero matrix)

  csc_set_data(P, 144, 144, P_nnz, P_x, P_i, P_p);
  ...
  OSQPFloat A_x[1] = { 0 };
  OSQPInt A_p[145] = { 0 };
  csc_set_data(A, 232, 144, A_nnz, A_x, A_i, A_p);
  */
